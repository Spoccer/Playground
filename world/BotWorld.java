...
//            System.out.println("INIT in BotWorld");
    ...
    {
        BotWorldGrid<GameObject> gr = (BotWorldGrid<GameObject>)this.getGrid();
...
//                        System.out.println(finishTime+"-"+startTime);
        ...
        if(((BotWorldGrid)gr).isMessageWaiting())
            setMessage(((BotWorldGrid)gr).getMessage());
        ...
        for (int r = 0; r < gr.getNumRows(); r++)
        {
            for (int c = 0; c < gr.getNumCols(); c++)
            {
                // If there's an object at this location, put it in the array.
                Location loc = new Location(r, c);
                Business b = ((BotWorldGrid)gr).getBusiness(loc);
                GameObject go = gr.get(loc);
                if(b!=null)
                {
                    b.decreaseDuration();
                    if(go instanceof Bot || go instanceof Customer)
                    {
                        if(!b.getColor().equals(go.getColor()))
                        {
                            if(hm.containsKey(b.getColor()))
                            {
                                int amount = hm.get(b.getColor());
                                hm.replace(b.getColor(), amount+b.getChargeAmount());        
                            }
                            else
                                hm.put(b.getColor(), b.getChargeAmount());
                        }
        ...
        {
            if(a instanceof Bot)
                if(hm.containsKey(a.getColor()))
                {
                    ((Bot)a).addToScore(hm.get(a.getColor()));
                }
        ...
        if(Math.random()<.12)
        {
            Location custLoc = RatBotsArena.getNewRandomSubwayStation(new Location(100,100));
            if(gr.get(custLoc) == null)
            {
                Customer bob = new Customer();
                bob.putSelfInGrid(gr, custLoc);
            }            
        }
        //--------ADD A Temporary Job Site --------------------------------------
        BotWorldGrid bwg = (BotWorldGrid)gr;
        if(Math.random()<.02)
        {
            Location jobLoc = this.getRandomEmptyLocation();
            if(gr.get(jobLoc) == null)
            {
                if(bwg.getBusiness(jobLoc) == null || 
                        ( bwg.getBusiness(jobLoc).getColor().equals(RatBotsColorScheme.getNeutralColor()) &&
                          bwg.getBusiness(jobLoc).getChargeAmount() < 1000 &&
                          bwg.getBusiness(jobLoc).getChargeAmount() > 0))
                {
                    bwg.setBusiness(jobLoc, new Business());
                    int val = -((int)(Math.random()*76)+125);
                    bwg.getBusiness(jobLoc).increaseChargeAmount(val);
                    bwg.getBusiness(jobLoc).setDuration((int)(Math.random()*15)+25);
                }
            }            
        }
        //----------Process Advertisements ------------------------------------
        ArrayList<Advertisement> ads = bwg.getAds();
        for(Advertisement ad : ads)
        {
            double influenceProportion = 2*Math.sqrt(ad.getAmount())/100;
//            System.out.println("Checking an AD = "+influenceProportion);
            //For each ad, each Customer has a % chance of going to this Location.  
            for (GameObject a : actors)
            {
                if(a instanceof Customer)
                {
                    if(Math.random()<=influenceProportion)
                    {
                        Customer c = (Customer) a;
                        c.setDetination(ad.getLocation());
//                        System.out.println("Influenced a Customer");
                    }   
//                    else System.out.println(" Didn't Influence a Customer");
                }
            }
        }
        bwg.clearAds();
...
//        Location inCenter = this.getRandomEmptyCenterLocation();
    ...
    public boolean isNearRat(Location loc)
    {
        ...
        BotWorldGrid grid = (BotWorldGrid)this.getGrid();
